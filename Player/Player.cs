...
namespace Player
{
    public class GamePlayer
    {
        ...
        public Soldier FindCurrentSoldier(Point i_SourcePoint)
        {
            ...
            for (int i = 0; i < PlayersSoldier.Count; ++i)
            {
                if (PlayersSoldier[i].X == i_SourcePoint.X && PlayersSoldier[i].Y == i_SourcePoint.Y)
                {
                    currentSoldier = PlayersSoldier[i];
                }
            }
        ...
        public void InitialSoldiers(ref Point io_SoldierPoint, int i_BoardSize, int i_PlayerIndex)
        {
            ...
            PlayersSoldier = new List<Soldier>(soldierCapacity);
            PlayerSignOnBoard = (ePlayerSignOnBoard)i_PlayerIndex;

            for (int i = 0; i < soldierCapacity; ++i)
            {
                PlayersSoldier.Add(new Soldier());
                PlayersSoldier[i].Y = i_BoardSize - 1 - io_SoldierPoint.Y;
                PlayersSoldier[i].X = io_SoldierPoint.X;
